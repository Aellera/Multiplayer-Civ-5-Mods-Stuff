-- CivilopediaScreen
-- Author: Tyler
-- DateCreated: 2/24/2013 9:45:40 PM
--------------------------------------------------------------
-------------------------------------------------
-- Civilopedia screen
-------------------------------------------------
...
CivilopediaCategory[CategoryBuildings].PopulateList = function()
	...
	local sql = [[
		SELECT Buildings.ID, Buildings.Description, Buildings.PortraitIndex, Buildings.IconAtlas, Buildings.PediaVisible --Buildings.PediaVisible added by trystero
		FROM Buildings 
		...
		WHERE FaithCost == 0 AND BuildingClasses.MaxGlobalInstances < 0 AND BuildingClasses.MaxPlayerInstances <> 1 AND BuildingClasses.MaxTeamInstances < 0 AND Technologies.Era = ? AND Buildings.PediaVisible = 1;]]; -- AND Buildings.PediaVisible = 1 added by trystero
	...
	for era in GameInfo.Eras() do
		...
		--PediaVisible added by trystero49
		...
		if(eraID == 1) then
			local sql = [[
				...
				FROM Buildings 
				INNER JOIN BuildingClasses ON Buildings.BuildingClass = BuildingClasses.Type 
				WHERE Buildings.PrereqTech IS NULL AND Buildings.FaithCost == 0 AND BuildingClasses.MaxGlobalInstances < 0 AND BuildingClasses.MaxPlayerInstances <> 1 AND BuildingClasses.MaxTeamInstances < 0 AND Buildings.PediaVisible = 1;]]; --Buildings.Pediavisible = 1 added by trystero
